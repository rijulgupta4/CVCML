...
OUTPUT_PATH = os.path.join(PROJECT_PATH, "Outputs", "Run 3 (Leakage Robustness)")
...
def save_feature_importance(model, feature_cols, model_name):
    ...
    clean_name = model_name.lower().replace(" ", "_").replace("+", "plus").replace("/", "_")
    out_file = os.path.join(OUTPUT_PATH, f"{clean_name}_feature_importance.csv")
...
def plot_curves(curve_results):
    ...
    plt.title("CLABSI Leakage/Ablation ROC Curves")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_PATH, "leakage_robustness_roc_curves.png"), dpi=300)
    plt.close()
    ...
    plt.title("CLABSI Leakage/Ablation Precision-Recall Curves")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_PATH, "leakage_robustness_pr_curves.png"), dpi=300)
    plt.close()


def add_ref_dwell_feature(df):
    df = df.copy()
    ...
    df["dwell_at_ref_hours"] = (
        (df["ref_time"] - df["starttime"]).dt.total_seconds() / 3600
    )
    df["dwell_at_ref_hours"] = df["dwell_at_ref_hours"].clip(lower=0)
    df["post_ref_dwell_hours"] = df["dwell_hours"] - df["dwell_at_ref_hours"]

    return df


def make_feature_set(base_features, remove_cols=None, add_cols=None):
    remove_cols = set(remove_cols or [])
    add_cols = list(add_cols or [])

    cols = [col for col in base_features if col not in remove_cols]
    for col in add_cols:
        if col not in cols:
            cols.append(col)
    return cols
...
# %% Load feature matrix and diagnose dwell leakage

print("Loading feature matrix...")
df = pd.read_csv(FEATURE_FILE)
df = add_ref_dwell_feature(df)
...
dwell_diagnostic = (
    df.groupby("clabsi")[["dwell_hours", "dwell_at_ref_hours", "post_ref_dwell_hours"]]
    .agg(["count", "mean", "median", "min", "max"])
)
dwell_diagnostic_file = os.path.join(OUTPUT_PATH, "dwell_leakage_diagnostic.csv")
dwell_diagnostic.to_csv(dwell_diagnostic_file)

print("")
print("Dwell leakage diagnostic:")
print(dwell_diagnostic.round(2).to_string())
print(f"Saved: {dwell_diagnostic_file}")
...
target_col = "clabsi"
base_feature_cols = [col for col in df.columns if col not in meta_cols + [target_col, "dwell_at_ref_hours"]]

print("")
print("Base feature count:")
print(f"  Original features: {len(base_feature_cols)}")
print(f"  Has dwell_hours: {'dwell_hours' in base_feature_cols}")
print(f"  Has site_known:  {'site_known' in base_feature_cols}")
...
# %% Define robustness variants

feature_sets = {
    "Original total dwell + site_known": make_feature_set(base_feature_cols),
    "Ref dwell + site_known": make_feature_set(
        base_feature_cols,
        remove_cols=["dwell_hours"],
        add_cols=["dwell_at_ref_hours"],
    ),
    "Original total dwell no site_known": make_feature_set(
        base_feature_cols,
        remove_cols=["site_known"],
    ),
    "Ref dwell no site_known": make_feature_set(
        base_feature_cols,
        remove_cols=["dwell_hours", "site_known"],
        add_cols=["dwell_at_ref_hours"],
    ),
    "No dwell no site_known": make_feature_set(
        base_feature_cols,
        remove_cols=["dwell_hours", "site_known"],
    ),
}
...
print("")
print("Robustness variants:")
for name, cols in feature_sets.items():
    print(f"  {name:<35} {len(cols)} features")
...
for model_name, feature_cols in feature_sets.items():
    ...
    clean_name = model_name.lower().replace(" ", "_").replace("+", "plus").replace("/", "_")
    model_file = os.path.join(MODEL_PATH, f"{clean_name}.joblib")
    joblib.dump({
        "model": model,
...
summary_file = os.path.join(OUTPUT_PATH, "leakage_robustness_model_comparison.csv")
...
threshold_file = os.path.join(OUTPUT_PATH, "leakage_robustness_threshold_table.csv")
threshold_out.to_csv(threshold_file, index=False)

plot_curves(curve_results)

print("")
print("Leakage/robustness comparison, sorted by PR-AUC:")
display_cols = [
    "model",
    "roc_auc",
    "pr_auc",
...
print(f"  Dwell diagnostic: {dwell_diagnostic_file}")
...
if "No dwell" not in best_model_name:
    try:
        import shap

        print("")
        print(f"Generating SHAP summary for {best_model_name}...")
        clean_name = best_model_name.lower().replace(" ", "_").replace("+", "plus").replace("/", "_")
        best_bundle = joblib.load(os.path.join(MODEL_PATH, f"{clean_name}.joblib"))
        ...
        plt.tight_layout()
        shap_file = os.path.join(PLOT_PATH, "leakage_robustness_best_model_shap.png")
...
print("")
print("Leakage robustness run complete.")
print("Interpretation target: performance should remain acceptable after replacing dwell_hours with dwell_at_ref_hours.")
