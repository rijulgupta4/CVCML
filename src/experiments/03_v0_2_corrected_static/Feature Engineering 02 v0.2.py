...
cohort = pd.read_csv(
    os.path.join(DATA_PATH, "clabsi_cohort_v0_2.csv"),
...
print(f"  CLABSI+ uses culture_time: {(cohort['clabsi'] == 1).sum():,}")
print(f"  CLABSI- uses endtime:      {(cohort['clabsi'] == 0).sum():,}")
print("")
...
# %% Save final v0.2 feature matrix

output_file = os.path.join(DATA_PATH, "clabsi_features_v0_2.csv")
...
print(f"CLABSI positive:  {cohort_encoded['clabsi'].sum():,} ({cohort_encoded['clabsi'].mean() * 100:.1f}%)")
print("")
print("Feature Engineering 02 v0.2 complete.")
