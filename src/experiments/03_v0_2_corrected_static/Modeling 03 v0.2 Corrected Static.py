...
OUTPUT_PATH = os.path.join(PROJECT_PATH, "Outputs", "Run 4.1 (v0.2 Corrected Static No Audit Leakage)")
...
FEATURE_FILE = os.path.join(DATA_PATH, "clabsi_features_v0_2.csv")
RANDOM_STATE = 42
...
def plot_curves(curve_results):
    ...
    plt.title("v0.2 Corrected Static ROC Curves (No Audit Leakage)")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_PATH, "v0_2_corrected_static_roc_curves.png"), dpi=300)
    plt.close()
    ...
    plt.title("v0.2 Corrected Static Precision-Recall Curves (No Audit Leakage)")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_PATH, "v0_2_corrected_static_pr_curves.png"), dpi=300)
    plt.close()
...
# %% Load v0.2 feature matrix

print("Loading v0.2 feature matrix...")
df = pd.read_csv(FEATURE_FILE)

print(f"Dataset shape: {df.shape}")
print(f"CLABSI positive: {df['clabsi'].sum():,} ({df['clabsi'].mean() * 100:.1f}%)")
print(f"CLABSI negative: {(df['clabsi'] == 0).sum():,}")
...
lab_cols = [
    ...
    if col.startswith(("creatinine_", "hemoglobin_", "lactate_", "platelets_", "wbc_"))
]
routine_lab_cols = [
    col for col in base_feature_cols
...
print("")
print(f"Feature columns after excluding total dwell_hours: {len(base_feature_cols)}")
...
feature_sets = {
    "v0.2 full corrected": make_feature_set(base_feature_cols),
    "v0.2 no site_known": make_feature_set(base_feature_cols, remove_cols=["site_known"]),
    "v0.2 no lactate": make_feature_set(base_feature_cols, remove_cols=set(base_feature_cols) - set(non_lactate_cols)),
    "v0.2 routine labs only": make_feature_set(
        base_feature_cols,
        remove_cols=set(base_feature_cols) - set(routine_lab_cols + ["dwell_at_ref_hours", "site_known", "anchor_age"]),
    ),
    "v0.2 no dwell no site": make_feature_set(
        base_feature_cols,
...
for name, cols in feature_sets.items():
    print(f"  {name:<28} {len(cols)} features")
...
summary_file = os.path.join(OUTPUT_PATH, "v0_2_corrected_static_model_comparison.csv")
...
threshold_file = os.path.join(OUTPUT_PATH, "v0_2_corrected_static_threshold_table.csv")
threshold_out.to_csv(threshold_file, index=False)

plot_curves(curve_results)

print("")
print("v0.2 corrected static comparison:")
display_cols = [
    "model",
    "roc_auc",
    "pr_auc",
...
try:
    ...
    plt.tight_layout()
    shap_file = os.path.join(PLOT_PATH, "v0_2_corrected_static_best_model_shap.png")
...
print("")
print("Modeling 03 v0.2 Corrected Static complete.")
