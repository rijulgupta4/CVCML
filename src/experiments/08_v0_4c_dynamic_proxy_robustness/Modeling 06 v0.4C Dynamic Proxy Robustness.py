...
OUTPUT_PATH = os.path.join(PROJECT_PATH, "Outputs", "Run 9 (v0.4C Dynamic Proxy Robustness)")
...
def plot_best_pr_curves(best_curve_results, label_frame):
    ...
    plt.title(f"v0.4C {label_frame} Best PR Curves")
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(
        os.path.join(PLOT_PATH, f"v0_4c_{clean_name(label_frame)}_best_pr_curves.png"),
        dpi=300,
    )
    plt.close()
...
def plot_horizon_summary(best_by_horizon, label_frame):
    ...
    plt.ylabel("Metric")
    plt.title(f"v0.4C {label_frame} Performance by Horizon")
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        os.path.join(PLOT_PATH, f"v0_4c_{clean_name(label_frame)}_performance_by_horizon.png"),
        dpi=300,
    )
    plt.close()
...
intensity_cols = [col for col in numeric_cols if any(token in col for token in intensity_tokens)]
value_cols = [col for col in numeric_cols if col not in intensity_cols]
...
feature_sets = {
    "v0.4C full values no intensity": value_cols,
    "v0.4C no site no intensity": value_no_site_cols,
    "v0.4C routine labs + vitals values": sorted(set(routine_lab_value_cols + vital_value_cols + context_cols)),
    "v0.4C routine labs + vitals values no site": sorted(set(routine_lab_value_cols + vital_value_cols + context_no_site_cols)),
    "v0.4C vitals values only": sorted(set(vital_value_cols + context_cols)),
    "v0.4C labs values only": sorted(set(lab_value_cols + context_cols)),
    "v0.4C context only": [col for col in numeric_cols if not col.startswith(lab_prefixes) and not col.startswith(vital_prefixes)],
}

feature_audit = pd.DataFrame([{
    "total_numeric_features_run8": len(numeric_cols),
    "measurement_intensity_features_removed": len(intensity_cols),
    "value_features_remaining": len(value_cols),
...
feature_audit_file = os.path.join(OUTPUT_PATH, "v0_4c_proxy_feature_audit.csv")
...
print("")
print(f"Measurement-intensity features excluded from value-only sets: {len(intensity_cols)}")
...
def make_label_frame(df, horizon, label_frame):
    ...
    out = df.copy()
    out["run9_target"] = out[target_col].astype(int)
    out["run9_label_frame"] = label_frame
...
summary_file = os.path.join(OUTPUT_PATH, "v0_4c_dynamic_proxy_model_comparison.csv")
...
best_file = os.path.join(OUTPUT_PATH, "v0_4c_dynamic_proxy_best_by_frame_horizon.csv")
best_by_frame_horizon.to_csv(best_file, index=False)

threshold_table = pd.DataFrame(threshold_rows)
threshold_file = os.path.join(OUTPUT_PATH, "v0_4c_dynamic_proxy_threshold_table.csv")
threshold_table.to_csv(threshold_file, index=False)

stay_summary = pd.DataFrame(stay_rows)
stay_file = os.path.join(OUTPUT_PATH, "v0_4c_dynamic_proxy_stay_level_summary.csv")
...
calibration_file = os.path.join(OUTPUT_PATH, "v0_4c_dynamic_proxy_calibration_deciles.csv")
...
try:
    ...
    df = make_label_frame(df_raw, horizon, label_frame)
    _, _, test_df = make_subject_level_splits(df, "subject_id", "run9_target")
    bundle = joblib.load(
    ...
    plt.tight_layout()
    shap_file = os.path.join(PLOT_PATH, "v0_4c_dynamic_proxy_best_model_shap.png")
...
manifest_rows = [
    ("Feature/proxy audit", feature_audit_file),
    ...
    ("Standard PR curves", os.path.join(PLOT_PATH, "v0_4c_standard_best_pr_curves.png")),
    ("Gray-zone excluded PR curves", os.path.join(PLOT_PATH, "v0_4c_gray_zone_excluded_best_pr_curves.png")),
    ("Best model SHAP", os.path.join(PLOT_PATH, "v0_4c_dynamic_proxy_best_model_shap.png")),
]
manifest_file = os.path.join(OUTPUT_PATH, "v0_4c_dynamic_proxy_output_manifest.csv")
pd.DataFrame(manifest_rows, columns=["output", "path"]).to_csv(manifest_file, index=False)

print("")
print("v0.4C dynamic proxy-robustness best model by label frame and horizon:")
display_cols = [
    "label_frame", "horizon_hours", "model", "roc_auc", "pr_auc", "brier_score",
    "base_rate", "threshold", "recall_sensitivity", "specificity", "precision_ppv",
...
print("")
print("Modeling 06 v0.4C Dynamic Proxy Robustness complete.")
