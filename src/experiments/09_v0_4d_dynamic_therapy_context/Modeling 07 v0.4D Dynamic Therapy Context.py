...
OUTPUT_PATH = os.path.join(PROJECT_PATH, "Outputs", "Run 10 (v0.4D Dynamic Therapy Context)")
...
def make_label_frame(df, horizon, label_frame):
    ...
    out = df.copy()
    out["run10_target"] = out[target_col].astype(int)
    out["run10_label_frame"] = label_frame
...
def plot_best_pr_curves(best_curve_results, label_frame):
    ...
    plt.title(f"v0.4D {label_frame} Best PR Curves")
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(
        os.path.join(PLOT_PATH, f"v0_4d_{clean_name(label_frame)}_best_pr_curves.png"),
        dpi=300,
    )
    plt.close()
...
def plot_horizon_summary(best_by_horizon, label_frame):
    ...
    plt.ylabel("Metric")
    plt.title(f"v0.4D {label_frame} Performance by Horizon")
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        os.path.join(PLOT_PATH, f"v0_4d_{clean_name(label_frame)}_performance_by_horizon.png"),
        dpi=300,
    )
    plt.close()
...
# %% Load v0.4D feature matrix

print("Loading v0.4D dynamic therapy-context feature matrix...")
...
therapy_prefixes = ("antibiotic_", "vasopressor_", "therapy_")
therapy_cols = [col for col in numeric_cols if col.startswith(therapy_prefixes)]
antibiotic_cols = [col for col in therapy_cols if col.startswith(("antibiotic_", "therapy_any_antibiotic", "therapy_broad_antibiotic"))]
vasopressor_cols = [col for col in therapy_cols if col.startswith(("vasopressor_", "therapy_any_vasopressor", "therapy_active_vasopressor"))]
...
feature_sets = {
    "v0.4D proxy-robust baseline no therapy": unique_cols([c for c in value_cols if not c.startswith(therapy_prefixes)]),
    "v0.4D therapy + context": unique_cols(therapy_cols + context_cols),
    "v0.4D therapy + context no site": unique_cols(therapy_cols + context_no_site_cols),
    "v0.4D antibiotics + context": unique_cols(antibiotic_cols + context_cols),
    "v0.4D vasopressors + context": unique_cols(vasopressor_cols + context_cols),
    "v0.4D vitals + therapy": unique_cols(vital_value_cols + therapy_cols + context_cols),
    "v0.4D labs + therapy": unique_cols(lab_value_cols + therapy_cols + context_cols),
    "v0.4D routine labs + vitals + therapy": unique_cols(routine_lab_value_cols + vital_value_cols + therapy_cols + context_cols),
    "v0.4D full values + therapy": value_cols,
    "v0.4D full values + therapy no site": value_no_site_cols,
}
...
feature_audit = pd.DataFrame([{
    ...
    "antibiotic_features": len(antibiotic_cols),
...
feature_audit_file = os.path.join(OUTPUT_PATH, "v0_4d_therapy_context_feature_audit.csv")
feature_audit.to_csv(feature_audit_file, index=False)

print("")
print("Feature sets:")
for name, cols in feature_sets.items():
    print(f"  {name:<48} {len(cols)} features")
print("")
print(f"Therapy features added: {len(therapy_cols)}")
...
summary_file = os.path.join(OUTPUT_PATH, "v0_4d_therapy_context_model_comparison.csv")
...
best_file = os.path.join(OUTPUT_PATH, "v0_4d_therapy_context_best_by_frame_horizon.csv")
best_by_frame_horizon.to_csv(best_file, index=False)

threshold_table = pd.DataFrame(threshold_rows)
threshold_file = os.path.join(OUTPUT_PATH, "v0_4d_therapy_context_threshold_table.csv")
threshold_table.to_csv(threshold_file, index=False)

stay_summary = pd.DataFrame(stay_rows)
stay_file = os.path.join(OUTPUT_PATH, "v0_4d_therapy_context_stay_level_summary.csv")
...
calibration_file = os.path.join(OUTPUT_PATH, "v0_4d_therapy_context_calibration_deciles.csv")
...
try:
    ...
    df = make_label_frame(df_raw, horizon, label_frame)
    _, _, test_df = make_subject_level_splits(df, "subject_id", "run10_target")
    bundle = joblib.load(
    ...
    plt.tight_layout()
    shap_file = os.path.join(PLOT_PATH, "v0_4d_therapy_context_best_model_shap.png")
...
manifest_rows = [
    ...
    ("Standard PR curves", os.path.join(PLOT_PATH, "v0_4d_standard_best_pr_curves.png")),
    ("Gray-zone excluded PR curves", os.path.join(PLOT_PATH, "v0_4d_gray_zone_excluded_best_pr_curves.png")),
    ("Best model SHAP", os.path.join(PLOT_PATH, "v0_4d_therapy_context_best_model_shap.png")),
]
manifest_file = os.path.join(OUTPUT_PATH, "v0_4d_therapy_context_output_manifest.csv")
pd.DataFrame(manifest_rows, columns=["output", "path"]).to_csv(manifest_file, index=False)

print("")
print("v0.4D dynamic therapy-context best model by label frame and horizon:")
display_cols = [
    "label_frame",
    "horizon_hours",
    "model",
...
print("")
print("Modeling 07 v0.4D Dynamic Therapy Context complete.")
