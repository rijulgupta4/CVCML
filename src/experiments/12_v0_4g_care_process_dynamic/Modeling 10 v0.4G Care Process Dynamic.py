...
OUTPUT_PATH = os.path.join(PROJECT_PATH, "Outputs", "Run 13 (v0.4G Care Process Dynamic)")
...
def make_label_frame(df, horizon, label_frame):
    ...
    out = df.copy()
    out["run13_target"] = out[target_col].astype(int)
    out["run13_label_frame"] = label_frame
...
feature_audit.to_csv(
    os.path.join(OUTPUT_PATH, "v0_4g_care_process_dynamic_feature_audit.csv"),
    index=False,
)

print("")
print("Run 13 feature sets:")
...
model_file = os.path.join(OUTPUT_PATH, "v0_4g_care_process_dynamic_model_comparison.csv")
...
best_file = os.path.join(OUTPUT_PATH, "v0_4g_care_process_dynamic_best_by_frame_horizon.csv")
best_by_frame_horizon.to_csv(best_file, index=False)

threshold_table = pd.DataFrame(threshold_rows)
threshold_file = os.path.join(OUTPUT_PATH, "v0_4g_care_process_dynamic_threshold_table.csv")
threshold_table.to_csv(threshold_file, index=False)

policy_summary = pd.DataFrame(policy_rows)
policy_file = os.path.join(OUTPUT_PATH, "v0_4g_care_process_dynamic_alert_policy_summary.csv")
...
calibration_file = os.path.join(OUTPUT_PATH, "v0_4g_care_process_dynamic_calibration_deciles.csv")
calibration_table.to_csv(calibration_file, index=False)

top_risk_table = pd.DataFrame(top_risk_rows)
top_risk_file = os.path.join(OUTPUT_PATH, "v0_4g_care_process_dynamic_top_risk_review_table.csv")
...
for label_frame in LABEL_FRAMES:
    ...
    plt.ylabel("Metric")
    plt.title(f"v0.4G {label_frame} Best Model Performance")
    plt.legend()
    plt.tight_layout()
    plt.savefig(
        os.path.join(PLOT_PATH, f"v0_4g_{clean_name(label_frame)}_performance_by_horizon.png"),
        dpi=300,
    )
    plt.close()
    ...
    plt.title(f"v0.4G {label_frame} Alert Policy Burden")
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(
        os.path.join(PLOT_PATH, f"v0_4g_{clean_name(label_frame)}_alert_policy_burden.png"),
        dpi=300,
    )
    plt.close()
...
try:
    ...
    df = make_label_frame(df_raw, horizon, label_frame)
    _, _, test_df = make_subject_level_splits(df, "subject_id", "run13_target")
    bundle = joblib.load(
    ...
    plt.tight_layout()
    shap_file = os.path.join(PLOT_PATH, "v0_4g_care_process_dynamic_best_model_shap.png")
...
manifest_rows = [
    ("Feature audit", os.path.join(OUTPUT_PATH, "v0_4g_care_process_dynamic_feature_audit.csv")),
    ...
    ("Best model SHAP", os.path.join(PLOT_PATH, "v0_4g_care_process_dynamic_best_model_shap.png")),
]
manifest_file = os.path.join(OUTPUT_PATH, "v0_4g_care_process_dynamic_output_manifest.csv")
...
print("")
print("v0.4G best calibrated model by label frame and horizon:")
...
print("")
print("Modeling 10 v0.4G Care Process Dynamic complete.")
