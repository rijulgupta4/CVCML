...
REPORT_DOCX = OUT / "CVCML Comprehensive Project Report - Runs 1-34 - Discussion Edition v0.7.docx"
...
def add_cover(doc):
    ...
    add_para(doc, "A discussion-oriented account of the clinical premise, data pipeline, model evolution, validation evidence, operational interpretation, and current limitations across Runs 1-34.",
    ...
    add_table(doc,
              ...
              [["Static and dynamic risk modeling", "Research prototype", "MIMIC-IV"],
               ["Runs 1-34", "Discussion Edition v0.7; not for clinical use", "External component checks: ARMD-MGB, eICU"]],
    ...
    add_para(doc, "Updated July 21, 2026", size=10.5, bold=True, color=DARK_BLUE)
    page_break(doc)
...
def add_exec_summary(doc):
    ...
    add_callout(doc, "The central result",
                "After prospectivity auditing, the final model retained modest and imprecise ranking signal but almost no demonstrated absolute-risk skill. This is partly a well-documented cautionary result: structured MIMIC-IV data did not support a dependable bedside predictor. Its narrow defensible role is retrospective prioritization of a bounded infection-prevention review queue.",
                fill=PALE_BLUE)
    ...
    add_table(doc,
              ...
              [
                  ...
                  ["AUPRC", "0.065 (0.041-0.113)", "1.54x the 4.25% event prevalence"],
                  ...
                  ["Project endpoint", "Run 34 complete", "Run 34 adds a same-frame logistic sensitivity analysis without reopening model selection"],
              ],
    ...
    for text in [
        ...
        "Evaluation emphasizes AUPRC lift, calibration, patient-clustered uncertainty, and review workload--not AUROC alone.",
    ...
    add_table(doc,
              ...
              [
                  ...
                  ["Appendix", "What happened in each of the 34 numbered runs and the Run 4.1/27.1 corrections?"],
...
def add_clinical_premise(doc):
    ...
    add_para(doc,
        "With event prevalence between roughly 1% and 5% depending on the prediction frame, a model can achieve a reasonable ROC-AUC while still producing many false positives. Precision-recall analysis directly connects recall to the fraction of flagged observations that are true positives and is therefore more informative for this imbalanced setting [11]. AUPRC, PPV, recall, false reviews per true positive, and review capacity are consequently central. The project treats ROC-AUC as one component of evidence, not the headline by itself.")
...
def add_data_methods(doc):
    ...
    add_para(doc,
        "The earliest versions selected a single eventual longest catheter per ICU stay. That choice uses future information and can attach cultures to the wrong exposure. Version 0.5 rebuilt the denominator from all recorded CVC procedure events and retained every eligible period. Within the same ICU stay, records separated by no more than four hours were merged into one continuous observed exposure; an episode ended at the earliest documented procedure end, ICU discharge, hospital discharge, or death. At least 48 hours of observed exposure were required before an episode entered the primary landmark cohort.")
    add_para(doc,
        "The four-hour tolerance was a pragmatic continuity rule intended to bridge short documentation gaps without joining clearly separated procedures. The 48-hour eligibility rule aligns the prediction cohort with the project outcome timing, while remaining an approximation rather than a complete NHSN line-day reconstruction. Both choices should be visible because EHR extraction and preparation decisions can materially change the cohort and prediction task [2,10]. They were prespecified for the final reconstruction but were not subjected to outcome-level sensitivity analyses across alternative merge gaps or duration thresholds. procedureevents is positive evidence that a line was documented; a missing record does not establish that no line was present [1].")
    add_table(doc,
    ...
    add_para(doc,
        "Run 17 converted eligible episodes into daily prediction rows. Each row represents what would have been knowable at a specific landmark, with a seven-day future outcome and explicit termination by line removal, procedure end, discharge, or death. This created 64,752 landmark rows before later source screening. Landmarking is appropriate because it updates predictions using information observed by a defined time among episodes still under observation; it therefore avoids using later measurements to describe an earlier prediction opportunity [9,10]. The daily seven-day frame also parallels prior dynamic CLABSI work, although the available data, endpoint, and implementation are not directly comparable [12].")

    add_heading(doc, "What Each Analysis Asks", 2)
    add_para(doc,
        "The project reports more than one analytical view because repeated landmarks and whole catheter episodes answer different questions. TRIPOD+AI recommends making the prediction time, target population, outcome, and intended use explicit [4]. The views below are complementary, not interchangeable performance estimates.")
    add_table(doc,
              ["Analysis view", "Unit and operation", "Question answered and rationale"],
              [
                  ["Landmark-row evaluation", "One eligible daily landmark; each row contributes equally", "Can the model rank a future-positive prediction opportunity above a future-negative opportunity using information available at that time? This is the primary dynamic-prediction estimand [9,10]."],
                  ["Episode-maximum evaluation", "One maximum score per reconstructed catheter episode", "Did an episode ever receive a high score? This summarizes episode ranking but uses a post-hoc maximum and is not a real-time absolute-risk estimate."],
                  ["Bounded review list", "Episodes ranked by maximum score; top 1%, 2%, 5%, 10%, or 20% reviewed", "How much proxy-positive yield is obtained for a fixed workload? PPV, recall, and false reviews are emphasized because the outcome is rare [11]."],
                  ["Episode-weighted sensitivity", "Each landmark weighted by the inverse of its episode landmark count", "Would long episodes dominate the row-level result? This is a sensitivity analysis, not a replacement for the primary estimand."],
              ], [1.45, 2.0, 2.85], font_size=8.7, first_col_bold=True)
    add_callout(doc, "Interpretation boundary",
                "The primary row-level result describes randomly selected prediction opportunities. The episode and worklist analyses describe retrospective prioritization. None estimates the probability of adjudicated NHSN CLABSI for an individual patient.",
                fill=PALE_GOLD)
...
def add_model_evolution(doc):
    ...
    add_para(doc,
        "The project should not be read as 34 attempts to tune the same classifier. It is a sequence of methodological corrections. Each phase changed what question was being answered and how closely the experiment resembled prospective use.")

    add_heading(doc, "Selected Milestones", 2)
    add_table(doc,
              ["Stage", "Representative result", "What was learned"],
              [
                  ["Runs 1-2: near-event static", "ROC-AUC 0.798-0.835; AUPRC 0.237-0.292", "Static and rolling-lab data contained strong retrospective signal, but reference times depended on eventual outcomes."],
                  ["Runs 4.1-6: corrected strict static", "Run 5 ROC-AUC 0.789; AUPRC 0.147", "Signal persisted under stricter organism logic, but calibration and alarm burden were poor."],
                  ["Runs 7-14: landmark dynamic", "Best v0.4H 168 h test ROC-AUC 0.706; AUPRC 0.039", "Vitals and therapy context added some ranking value, but event rarity dominated and bedside alerting remained impractical."],
                  ...
                  ["Runs 25-30: evaluation and leakage audit", "Final safe ROC-AUC 0.612; AUPRC 0.065", "Removing outcome-derived information changed AUPRC only modestly and preserved limited ranking signal."],
    ...
    add_para(doc,
        "The initial improved XGBoost model increased AUPRC from 0.157 to 0.237, and static-plus-lab optimization reached AUPRC 0.292. The apparent improvement was real within that retrospective task, but positive cases were characterized near culture while negatives were referenced near catheter removal. This made the model a near-event characterization tool rather than a deployable insertion-time baseline.")
    add_para(doc,
        "After correcting the audit feature and applying stricter organism logic, Run 5 achieved ROC-AUC 0.789 and AUPRC 0.147. Run 6 showed why discrimination was not enough: at the automatically selected threshold, the model would flag 38.9 of every 100 stays, with PPV 5.4% and approximately 17.5 false alerts per true positive.")
    ...
    add_para(doc,
        "Run 29 found that early_positive_culture encoded eventual culture positivity and could be present before the positive result was demonstrably available. The feature was removed from the final candidate. Validation AUPRC fell by only 0.0038, suggesting that the broader model signal was not solely created by this defect.")
...
def add_final_evidence(doc):
    ...
    add_table(doc,
              ...
              [
                  ...
                  ["AUPRC", "0.065", "0.041-0.113", "Above 0.0425 prevalence; 1.54x lift"],
    ...
    add_heading(doc, "Run 34: Same-Frame Logistic Sensitivity Analysis", 2)
    add_para(doc,
        "Run 34 asked whether the selected nonlinear model was obscuring a simpler, more stable relationship. An L2-regularized logistic regression used the same leakage-safe predictors, eligibility rules, source-screened seven-day target, development years (2008-2013), calibration years (2014-2016), and 2017-2019 characterization frame as the final XGBoost candidate. The repeatedly inspected 2020-2022 period was neither loaded nor scored. This was a post-hoc comparator: it did not reselect the primary model, revise the threshold, or reopen broad tuning.")
    add_table(doc,
              ["Analysis unit", "XGBoost", "Logistic regression", "Paired interpretation"],
              [
                  ["Landmark rows", "AUROC 0.612; AUPRC 0.065", "AUROC 0.620; AUPRC 0.059", "Neither paired difference excluded no difference"],
                  ["Episode maximum", "AUROC 0.662; AUPRC 0.161", "AUROC 0.648; AUPRC 0.125", "XGBoost ranked episodes somewhat better; AUPRC difference remained imprecise"],
                  ["Top 10% review", "PPV 15.4%; recall 18.5%", "PPV 12.3%; recall 14.8%", "XGBoost recovered 10 versus 8 positive episodes among 65 reviewed"],
                  ["Top 20% review", "PPV 16.2%; recall 38.9%", "PPV 11.5%; recall 27.8%", "XGBoost recovered 21 versus 15 positive episodes among 130 reviewed"],
              ], [1.3, 1.55, 1.55, 1.9], font_size=8.6, first_col_bold=True)
    add_para(doc,
        "At landmark level, the comparison did not establish that either model class was superior. XGBoost retained numerically better episode-level ranking and worklist yield, but the episode AUPRC difference was also compatible with no difference. The practical lesson is that model family was not the principal bottleneck; outcome validity, event count, temporal framing, and information quality constrained both approaches. Run 34 used a separate paired bootstrap, so its XGBoost confidence intervals differ slightly from the primary Run 30 intervals retained above.",
    ...
    add_figure(doc, FIGURES["policy"],
               ...
               width=6.15)
    add_callout(doc, "Discussion prompt",
                "Is a PPV near 15%--roughly five false reviews per true-positive episode--an acceptable tradeoff for a deliberately bounded infection-prevention worklist? What review capacity, action, or downstream benefit would change that answer?",
                fill=PALE_BLUE)
    page_break(doc)
...
def add_clinical_interpretation(doc):
    ...
    add_heading(doc, "What the Locked Errors Looked Like", 2)
    add_para(doc,
        "Run 26 examined errors in the previously inspected lockbox after model development. This was a post-hoc characterization exercise, not a replacement for the final 2017-2019 temporal evaluation. In that historical frame, the top-5% review list contained 17 true positives and 97 false positives; the top-10% list contained 31 true positives and 196 false positives, while 103 positive episodes remained outside the list.")
    add_table(doc,
              ["Run 26 contrast", "Observed pattern", "Interpretation"],
              [
                  ["Monitoring intensity", "False positives had higher platelet, WBC, and lactate measurement counts (standardized differences 0.74, 0.70, and 0.69)", "The score partly recognized clinician concern and testing intensity, not infection biology alone"],
                  ["Captured positives", "Included more E. faecium and Candida examples", "Some organism and severity patterns were easier for the model to rank"],
                  ["Missed positives", "Included S. epidermidis, Prevotella, and mixed anaerobic or gram-negative patterns", "Source-complex and heterogeneous episodes remained difficult"],
              ], [1.35, 2.55, 2.4], font_size=8.8, first_col_bold=True)
    add_callout(doc, "Discussion prompt",
                "Should measurement frequency be retained as useful context, constrained because it can encode clinician behavior, or separated from physiologic predictors in a future model?",
                fill=PALE_BLUE)

    add_heading(doc, "Agreement With Administrative CVC-BSI Coding", 2)
    add_para(doc,
        "Runs 27 and 27.1 tested whether the strict proxy behaved like an administrative CLABSI label. Across 22,812 catheter episodes, 291 were proxy-positive and 677 had an ICD-specific CVC-BSI code; only 60 episodes were positive by both definitions, corresponding to a Jaccard agreement of 6.6%. At admission level, agreement remained low at 8.5% (56 overlapping positive admissions). This gap should be read in both directions: the structured proxy is incomplete, but administrative coding is not an adjudicated surveillance gold standard.")
    add_table(doc,
              ["Agreement view", "Result", "Why it matters"],
              [
                  ["Episode level", "60 overlaps; Jaccard 6.6%", "Proxy and ICD coding identify substantially different groups"],
                  ["Admission level", "56 overlaps; Jaccard 8.5%", "Low agreement persists after reconciling episode grain"],
                  ["ICD-positive/proxy-negative", "46.7% had no qualifying blood culture; 43.3% had no >=48-hour eligible exposure", "Many disagreements arise from reconstructable timing or evidence differences"],
                  ["Proxy-positive/ICD-negative", "63.2% had nonconcordant source-culture evidence; 8.2% were primary-likely with no code", "Some cases remain source-uncertain, while others may be administratively missed"],
                  ["Organism examples", "S. epidermidis overlap 66.7%; E. faecium overlap 16.5%", "Agreement varies materially by organism"],
              ], [1.6, 1.85, 2.85], font_size=8.7, first_col_bold=True)
    add_para(doc,
        "Published validation work provides important context. In one UNC internal validation exercise, paired infection preventionists agreed on approximately two thirds of reviewed blood cultures, with secondary-source attribution among the common disagreements. Tennessee external NHSN validation reported high overall sensitivity but substantial facility-level variation, with secondary-BSI misattribution accounting for many missed reports. These studies do not validate the project proxy, but they show why disagreement with ICD coding cannot be assigned to one label alone [7,8].",
        size=9.3, color=MUTED, italic=True)
    add_callout(doc, "Discussion prompt",
                "For the intended research question, should the primary endpoint favor structured reproducibility, administrative comparability, or resource-intensive clinical adjudication--and which disagreements would matter most to resolve?",
                fill=PALE_BLUE)
    ...
    add_figure(doc, FIGURES["subgroups"],
               "Figure 5. AUPRC lift by prespecified subgroup. Blue cells met the more-stable reporting rule; orange cells require cautious interpretation.",
               width=6.1)
    page_break(doc)
...
def add_external_evidence(doc):
    ...
    add_para(doc,
        "The project tested two independent databases before forcing an external performance estimate. ARMD-MGB contained rich microbiology but no catheter exposure or longitudinal predictor timeline. In eICU, the feasibility funnel began with 244 positive-blood-culture stays, but only one met the explicit >=48-hour line-placement criterion under the attempted mapping. Neither database could honestly reproduce the frozen seven-day catheter-episode task.")
    add_table(doc,
    ...
    add_callout(doc, "Evidence boundary",
                ...
                fill=PALE_GOLD)
    add_callout(doc, "Discussion prompt",
                "Does the 244-to-1 eICU feasibility funnel argue for finding a richer external EHR, redesigning the task around more portable variables, or treating external cohort reconstructability itself as a principal result?",
                fill=PALE_BLUE)
    page_break(doc)
...
def add_current_state(doc):
    ...
    for text in [
        ...
        "Run 33 provides the consolidated evidence package and claim register; Run 34 adds a traceable same-frame logistic sensitivity analysis without changing the primary model.",
    ]:
        add_bullet(doc, text)
    ...
    add_callout(doc, "How the central result should be read",
                "The final result is partly negative and cautionary: after outcome-adjacent information was removed and prediction timing was made realistic, structured MIMIC-IV data did not yield a dependable absolute-risk model. That finding is useful because it identifies the true bottlenecks--outcome validity, catheter documentation, event scarcity, and transportability--rather than attributing the problem to insufficient tuning.",
                fill=PALE_GOLD)
    ...
    add_callout(doc, "Presentation version",
                "CVCML began as a high-performing static XGBoost model for catheter-associated bloodstream infection risk. The strongest early scores depended on retrospective timing, so we rebuilt the cohort around all line episodes, daily landmarks, and a seven-day source-screened outcome. The final leakage-safe model has modest discrimination and weak calibration, but it can enrich a bounded retrospective infection-prevention review list. A same-frame logistic sensitivity analysis produced similar, imprecise landmark performance, showing that model complexity was not the main limitation. Run 34 is the current project endpoint; future work should focus on independent label validation or external transportability, not more tuning.",
                fill=PALE_BLUE)
    ...
    for text in [
        ...
        "A simpler logistic model reached similar landmark performance, so model complexity was not the central bottleneck.",
...
RUNS = [
    ...
    ("2", "Static optimization", "Compared logistic regression, random forest, static XGBoost, tuned XGBoost, and calibration; tuning did not beat the untuned static-plus-lab AUPRC."),
    ...
    ("34", "Same-frame logistic comparator", "Compared L2-regularized logistic regression with the frozen leakage-safe XGBoost frame; landmark differences were inconclusive, while XGBoost retained numerically better episode ranking and review yield."),
]
...
def add_appendix(doc):
    ...
    add_table(doc,
              ...
              [
                  ...
                  ["AUPRC", "Average precision across thresholds; more informative than ROC-AUC when positives are rare."],
                  ["AUPRC lift", "AUPRC divided by event prevalence; values above 1 indicate enrichment over random ranking."],
    ...
    references = [
        ...
        "[7] DiBiase L, Teal L, Sickbert-Bennett Vivante E, et al. Implementing an internal CLABSI validation program. Antimicrobial Stewardship & Healthcare Epidemiology. 2024;4(Suppl 1):s24. doi:10.1017/ash.2024.128.",
        "[8] Roberts C, Thure KA, Reed V. External validation of NHSN CLABSI data in acute care hospitals. American Journal of Infection Control. 2020;48(8 Suppl):S54. doi:10.1016/j.ajic.2020.06.026.",
        "[9] van Houwelingen HC. Dynamic prediction by landmarking in event history analysis. Scandinavian Journal of Statistics. 2007;34:70-85. doi:10.1111/j.1467-9469.2006.00529.x.",
        "[10] Albu E, Gao S, Stijnen P, et al. Challenges and recommendations for electronic health records data extraction and preparation for dynamic prediction modeling in hospitalized patients: practical guide and tutorial. Journal of Medical Internet Research. 2025;27:e73987. doi:10.2196/73987.",
        "[11] Saito T, Rehmsmeier M. The precision-recall plot is more informative than the ROC plot when evaluating binary classifiers on imbalanced datasets. PLOS ONE. 2015;10:e0118432. doi:10.1371/journal.pone.0118432.",
        "[12] Albu E, Gao S, Stijnen P, et al. Hospital-wide, dynamic, individualized prediction of central line-associated bloodstream infections: development and temporal evaluation of six prediction models. BMC Infectious Diseases. 2025;25:597. doi:10.1186/s12879-025-10854-1.",
    ]
    for ref in references:
        p = add_para(doc, ref, size=8.2, color=INK, after=1.5, line=0.95)
        p.paragraph_format.left_indent = Inches(0.25)
        p.paragraph_format.first_line_indent = Inches(-0.25)

    add_heading(doc, "Appendix D. Project File Map", 1)
    for location, purpose in [
        ("src/", "Versioned extraction, feature engineering, modeling, audit, comparator, and report-building scripts"),
        ("Outputs/Run 1 ... Run 34/", "Run-specific metrics, plots, models, manifests, and notes"),
        ("Notebooks/CVCML Project Hub - updated v0.5 Run33.docx", "Detailed chronological project log through the consolidated evidence run"),
        ("Outputs/Run 33 (v0.5 Publication Consolidation)/", "Frozen leakage-safe XGBoost evidence and figures"),
        ("Outputs/Run 34 (v0.5 Same-Frame Logistic Comparator)/", "Post-hoc same-frame model-family sensitivity outputs"),
        ("Reports/CVCML Scientific Manuscript Draft/", "Scientific manuscript and supplementary information v0.7"),
    ]:
        p = doc.add_paragraph()
    ...
    add_para(doc, "End of report", size=9, bold=True, color=MUTED,
             align=WD_ALIGN_PARAGRAPH.CENTER, before=4, after=0)
...
def build():
    ...
    add_appendix(doc)
    doc.core_properties.title = "CVCML Comprehensive Project Report - Runs 1-34 - Discussion Edition v0.7"
